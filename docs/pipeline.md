...
For phasing and imputation, chrX is split into three independent chunks (PAR1, nonPAR, PAR2). These splits are then automatically merged by Helmholtz Munich Imputation Server and are returned as one complete chromosome X file. Only Eagle is supported.
